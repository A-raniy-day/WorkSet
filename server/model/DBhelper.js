...
*/
  ...
  SELECT_TABLE_BY_OR(tableName, where) {
    if (where != undefined) {
      let w = Object.keys(where)
        .map(k => {
          return typeof where[k] === "string"
...
*/
  ...
  */
  ...
  select1(tableName, where, cb) {
    if (!cb) {
      return this.query(this.SELECT_TABLE1(tableName, where));
    }
    this.query(this.SELECT_TABLE1(tableName, where)).then(res => {
      cb && cb(res);
    });
  }
  select_or(tableName, where, cb) {
    if (!cb) {
      return this.query(this.SELECT_TABLE_BY_OR(tableName, where));
    }
    this.query(this.SELECT_TABLE_BY_OR(tableName, where)).then(res => {
      cb && cb(res);
    });
  }
