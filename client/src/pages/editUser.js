...
export default class Order extends React.Component {
 ...
 getRandom = async () => {
   uid  =this.props.match.params.uid; 
    ...
    console.log(uid)
    //首先获取后端的值
     await axios.get(`http://localhost:3001/user/${uid}`).then(res=>{
  ...
  document.getElementById("infolabel").innerText=res.data[0].consignee+"("
  +res.data[0].phonenumber+")"+""+res.data[0].address;
  //姓名赋值
  document.getElementById("infoname").innerText=res.data[0].name;
  document.getElementById("infoidnumber").innerText=res.data[0].idnumber;
   }
   else{
     //不能搜出的情况
    this.setState({
...
  sure(){
   
  if(document.getElementById("buyerName").value.replace(/\s+/g,"") == "" 
  || document.getElementById("buyerPhoneNumber").value.replace(/\s+/g,"") == ""
  ||document.getElementById("username").value.replace(/\s+/g,"") == ""
  ||document.getElementById("idNumber").value.replace(/\s+/g,"") == ""
  ){
    alert("请将信息填写完整!");
}
...
  }
  axios.post(`http://localhost:3001/user/${uid}`,changefir).then((response) => {
  
  });
alert("修改信息成功！");
...
  ||document.getElementById("username2").value.replace(/\s+/g,"") == ""
  ||document.getElementById("idNumber2").value.replace(/\s+/g,"") == ""){
    alert("请将收货信息填写完整!");
}
...
    idnumber:document.getElementById("idNumber2").value ,
  }
  axios.post(`http://localhost:3001/user/${uid}`,changefir).then((response) => {
  
  });
...
<div>
      ...
      placeholder="请输入真实姓名" autocomplete="off"
      required  
      ></input>
  </div>
  <div>
      ...
      value={this.state.che} onChange={(p) => this.changeidnum(p)}
      ></input>
  </div>
<h3>收货地址</h3>
<div id = "info" >
...
<h3>用户信息</h3>
<p><label>真实姓名：</label><label id="infoname"></label></p>
 <p><label>身份证号：</label><label id="infoidnumber"></label></p>
<h3>收货地址</h3>
<input checked type="radio" name="sendinfo" value='inform'/>     
<label for="defaltAddress" id="infolabel">  </label>
<p></p>
<button class="btn1" onClick={this.changeClick}>修改</button>
<div id="changePage" >
...
<div>
      <label for="nickname">真实姓名</label>
      <input type="text" id ="username2" name ="username"  maxlength="10" 
      placeholder="请输入真实姓名" autocomplete="off"
      required  
      ></input>
  </div>
  <div>
      <label for="idNumber">身份证号</label>
      <input type="text" id ="idNumber2" name ="idNumber"  maxlength="20" 
      placeholder="请输入身份证号" autocomplete="off"
      value={this.state.che} onChange={(p) => this.changeidnum(p)}
      ></input>
  </div>
