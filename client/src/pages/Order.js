...
// const onSubmit = (data) => {
//   console.log(data);
//   axios.post(`http://localhost:3001/user/${uid}`, data).then((response) => {
...
export default class Order extends React.Component {
 ...
 getRandom = async () => {
   ...
   bids=this.props.match.params.bids;
    //查找用户信息，保持顶部的个人信息还在
    //首先获取后端的值
   if(bid!=undefined){
    await axios.get(`http://localhost:3001/order/${bid}`).then(res=>{
      if(res.data[0][0].address!=null)
   {
    this.setState({
      //有个人信息的情况
      showElem:'none',
      showElem1:'display',
      bookinfo:[res.data[1][0]],
  })
  bookinfo=this.state.bookinfo;
  uid=res.data[0][0].uid;
  document.getElementById("infolabel").innerText=res.data[0][0].consignee+"("
  +res.data[0][0].phonenumber+")"+""+res.data[0][0].address;
  document.getElementById("addressnew").innerText=res.data[0][0].address;
  document.getElementById("namenew").innerText=res.data[0][0].consignee;
  document.getElementById("phonenew").innerText=res.data[0][0].phonenumber;
  document.getElementById("mathlast").innerText=res.data[1].length;
  document.getElementById("pricelast").innerText=res.data[1][0].newprice;
   }
   else{
     //不能搜出的情况
    this.setState({
  ...
  {
price+=res.data[1][0].newprice;
  }
  document.getElementById("pricelast").innerText=price;
   }
   else{
     //不能搜出的情况
    this.setState({
...
addressClick() {
  ...
  if(document.getElementsByClassName("select")[2].value!="=请选择县区=")
  {
   adress1.pro=document.getElementsByClassName("select")[0].value;
   adress1.cit=document.getElementsByClassName("select")[1].value;
   adress1.coun=document.getElementsByClassName("select")[2].value;
   document.getElementById("addr-show").value=adress1.pro+"-"+adress1.cit+"-"+adress1.coun;
   let startText1 = document.getElementById("addr-show").value; //获取开头字符串
   document.getElementById("addr-show").onkeyup = function () {
//如果不是以startText1开头的，就把文本框内的值设为startText(强行删除emmm，感觉怪怪的，有机会可以改进一下)
(document.getElementById("addr-show").value.indexOf(startText1) === 0) || (document.getElementById("addr-show").value = startText1);
};
   document.getElementById("detailedAdress").value=adress1.pro+"-"+adress1.cit+"-"+adress1.coun+" ";
   let startText = document.getElementById("detailedAdress").value; //获取开头字符串
   document.getElementById("detailedAdress").onkeyup = function () {
//如果不是以startText开头的，就把文本框内的值设为startText(强行删除emmm，感觉怪怪的，有机会可以改进一下)
(document.getElementById("detailedAdress").value.indexOf(startText) === 0) || (document.getElementById("detailedAdress").value = startText);
};
  }
  else{
    document.getElementById("addr-show").value="";
    document.getElementById("detailedAdress").value="";
    let startText1 = document.getElementById("addr-show").value; //获取开头字符串
    document.getElementById("addr-show").onkeyup = function () {
 //如果不是以startText1开头的，就把文本框内的值设为startText(强行删除emmm，感觉怪怪的，有机会可以改进一下)
 (document.getElementById("addr-show").value.indexOf(startText1) === 0) || (document.getElementById("addr-show").value = startText1);
...
addressClick1() {
  ...
  if(document.getElementsByClassName("select1")[2].value!="=请选择县区=")
  {
   adress1.pro=document.getElementsByClassName("select1")[0].value;
   adress1.cit=document.getElementsByClassName("select1")[1].value;
   adress1.coun=document.getElementsByClassName("select1")[2].value;
   document.getElementById("addr-show2").value=adress1.pro+"-"+adress1.cit+"-"+adress1.coun;
   let startText1 = document.getElementById("addr-show2").value; //获取开头字符串
   document.getElementById("addr-show2").onkeyup = function () {
//如果不是以startText1开头的，就把文本框内的值设为startText(强行删除emmm，感觉怪怪的，有机会可以改进一下)
(document.getElementById("addr-show2").value.indexOf(startText1) === 0) || (document.getElementById("addr-show2").value = startText1);
};
   document.getElementById("detailedAdress2").value=adress1.pro+"-"+adress1.cit+"-"+adress1.coun+" ";
   let startText = document.getElementById("detailedAdress2").value; //获取开头字符串
   document.getElementById("detailedAdress2").onkeyup = function () {
//如果不是以startText开头的，就把文本框内的值设为startText(强行删除emmm，感觉怪怪的，有机会可以改进一下)
(document.getElementById("detailedAdress2").value.indexOf(startText) === 0) || (document.getElementById("detailedAdress2").value = startText);
};
  }
  else{
    document.getElementById("addr-show2").value="";
    document.getElementById("detailedAdress2").value="";
    let startText1 = document.getElementById("addr-show2").value; //获取开头字符串
    document.getElementById("addr-show2").onkeyup = function () {
 //如果不是以startText1开头的，就把文本框内的值设为startText(强行删除emmm，感觉怪怪的，有机会可以改进一下)
 (document.getElementById("addr-show2").value.indexOf(startText1) === 0) || (document.getElementById("addr-show2").value = startText1);
...
//控制输入的电话号码不能有奇怪的选项
changeEvent=(p)=>{
  let value = p.target.value.replace(/\D/g, '')
  this.setState({ checkCode: value })
}
//控制输入的电话号码不能有奇怪的选项
changeEvent1=(p)=>{ 
  let value = p.target.value.replace(/\D/g, '')
  this.setState({ checkCode1: value })
}
changeClick(){
  let showchange =  document.getElementById("changePage"); 
  showchange.style.display= 'block';
}
//这个时候的情况是输入上面的情况
  sure(){
   
  if(document.getElementById("buyerName").value.replace(/\s+/g,"") == "" || document.getElementById("buyerPhoneNumber").value.replace(/\s+/g,"") == ""){
    document.getElementById("buyerName").value="";
    //清空之前的值
    document.getElementById("buyerPhoneNumber").value = "";
    alert("请将信息填写完整!");
}
else if(document.getElementById("buyerPhoneNumber").value.length !=11){
alert("请填写正确手机号码！");
}
else{
  let changefir = {
    buyerName:document.getElementById("buyerName").value,
    buyerPhoneNumber:document.getElementById("buyerPhoneNumber").value,
    detailedAdress:document.getElementById("detailedAdress").value
    
  }
  axios.post(`http://localhost:3001/order/firstchange`,changefir).then((response) => {
  
  });
alert("修改收货信息成功！");
if(bid!=undefined){
  window.location.href=`http://localhost:3000/order/${bid}`;
}
else{
...
//已有想要修改 
  ...
  if(document.getElementById("buyerName2").value.replace(/\s+/g,"") == "" || document.getElementById("buyerPhoneNumber2").value.replace(/\s+/g,"") == ""){
    document.getElementById("buyerName2").value="";
    //清空之前的值
    document.getElementById("buyerPhoneNumber2").value = "";
    alert("请将信息填写完整!");
}
else if(document.getElementById("buyerPhoneNumber2").value.length !=11){
alert("请填写正确手机号码！");
}
else{
  let changefir = {
    buyerName:document.getElementById("buyerName2").value,
    buyerPhoneNumber:document.getElementById("buyerPhoneNumber2").value,
    detailedAdress:document.getElementById("detailedAdress2").value
...
if(bid!=undefined){
  ...
  buy(){
    ...
    axios.post(`http://localhost:3001/order/orderone`,bookinfo).then((response) => {
    
    });
    alert("下单成功！商品已经迫不及待的奔向小主了！")
   //重定向
...
return (
 ...
 <h1>支付</h1>
<h2>收货地址</h2>
<div id = "info" style={{display:this.state.showElem}}>
  <div>
            <label for="addr-show">选择所在地:
                          ...
                          {this.state.cities.length>0?this.state.cities.map(city => (
                           ...
                           <option>=请选择城市=</option>
                                  ...
                                  <select className="select" onChange={this.handleChange.bind(this, "county")}>
                                      {this.state.counties.length>0?this.state.counties.map(county => (
            ...
            <button type="button" class="met1 btn" onClick={this.addressClick}>确定</button>

  </div>
 <form  id ="firstadress">
  <div>
      <label for="detailedAdress">详细地址</label>
      <input type="text" id ="detailedAdress" name ="detailedAdress" 
      placeholder="请填写详细到门牌号的姓名" autocomplete="off" maxlength="100" 
      required/>
  </div>
  <div>
      <label for="buyerName">收货人姓名</label>
      <input type="text" id ="buyerName" name ="buyerName" 
      placeholder="请输入收货人姓名" autocomplete="off"maxlength="20" 
      required/>
  </div>
  <div>
      <label for="buyerPhoneNumber">联系电话</label>
      <input type="text" id ="buyerPhoneNumber" name ="buyerPhoneNumber"  maxlength="11" 
      placeholder="请输入收货联系号码" autocomplete="off"
      required  value={this.state.checkCode} onChange={(p) => this.changeEvent(p)}
      ></input>
  </div>
  <div>
    <button type="button" class="met1 btn" id="verifyInfo"onClick={this.sure}>确认</button>
  </div>
  </form>
</div>

<div  style={{display:this.state.showElem1}}>
<input checked type="radio" name="sendinfo" value='inform'/>     
<label for="defaltAddress" id="infolabel">  </label>
<button id="changeButton" onClick={this.changeClick}>修改</button>
<div id="changePage" >
 <div class = "modal_content" >
   <div class="modal-header">
    <button class="close_Button" id="close_Button"onClick={this.closechange} >&times;</button>
    <h2>修改</h2>
   </div>
<div class="modal-body">
  <div>
    <label for="addr-show">选择所在地：
      <input type="text"  id="addr-show2"></input>
    </label>
    <br></br>
    <select className="select1" onChange={this.handleChange.bind(this, "province")} >
                                      {this.state.provinces.map(province => (
                                          <option value={province} key={id1++}>{province}</option>
                                      ))
                                    }
            </select>
                     <select className="select1" onChange={this.handleChange.bind(this, "city")}>
                          {this.state.cities.length>0?this.state.cities.map(city => (
                                  <option value={city} key={id1++} >{city}</option>
                           )):
                           <option>=请选择城市=</option>
                          }
                           
                                  </select>
                                  <select className="select1" onChange={this.handleChange.bind(this, "county")}>
                                      {this.state.counties.length>0?this.state.counties.map(county => (
                                          <option value={county} key={id1++}>{county}</option>
                                      )):
                                      <option>=请选择县区=</option>
                                    }
                                  </select>

    <button type="button" class="met1 btn" id ="btnforcity"onClick={this.addressClick1}>确定</button>
    <form action="/pay2" method = 'POST' id="infoForm2">
    <div>
      <label for="detailedAdress">详细地址</label>
      <input type="text" id ="detailedAdress2" name ="detailedAdress2" 
      placeholder="请填写详细到门牌号的姓名" autocomplete="off"maxlength="100" 
      required/>
  </div>
  <div>
      <label for="buyerName">收货人姓名</label>
      <input type="text" id ="buyerName2" name ="buyerName2" 
      placeholder="请输入收货人姓名" autocomplete="off"maxlength="20" 
      required/>
  </div>
  <div>
      <label for="buyerPhoneNumber">联系电话</label>
      <input type="text" id ="buyerPhoneNumber2" name ="buyerPhoneNumber2"  maxlength="11" 
      placeholder="请输入收货联系号码" autocomplete="off" value={this.state.checkCode1}onChange={(p) => this.changeEvent1(p)}/>
  </div>
  <div>
    <button type="button" class="met1 btn" id="verifyInfo2" onClick={this.sure1}>确认</button>
  </div>
  </form>
</div>
</div>
...
{this.state.bookinfo.map(book => (
                                   <div key={id1++}>
                                     <p>店主：<Link to={"/user/"+book.uid}  >店主主页</Link></p>
                                   <hr />
                                  <div class="booo"  >
                                    <div class="book-imgnew"> <img src={book.url} class="bookinfomationnewurl"></img></div>
                                    <div class="booknnn"><p><Link to={"/book/"+book.bid}  >{book.bookname}</Link></p></div>
                                    <div class="bookinfomationage"><p><span>{book.aging_degree}</span></p></div>
                                    <div class="bookinfomationnewprice"><p><span >￥{book.newprice}</span></p></div>
...
<div class="sss">
<img src={car} width="40px" height="30px" ></img>
<label for="sendMethod">运送方式:</label>
<input checked type="radio" name="sendWay" value='package'/>快递
</div>
<div>
<label for="remind">共计 </label>
<label for="count"  id="mathlast"></label>
<label for="remind"> 件商品，应付总金额 ￥</label>
<label for="price" id="pricelast"></label>
</div>
<div>
 <label for="remindd" >配送至：</label>
 <label id="addressnew" class="ssss"></label>
 <label for="name">，<label id="namenew"class="ssss"></label>收，</label>
 <label for="phone" id="phonenew"class="ssss">电话</label>
</div>
<div class="war">
  <img src={warning} id="warningimg"></img>
</div>

  <button type = "submit" onClick={this.buy} class="submitorders">确认并提交订单</button>
